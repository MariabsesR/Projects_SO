...
/* Função que escreve uma operação no buffer de memória partilhada entre os
 * clientes e intermediários. A operação deve ser escrita numa posição livre do
 * buffer, tendo em conta o tipo de buffer e as regras de escrita em buffers
 * desse tipo. Se não houver nenhuma posição livre, não escreve nada.
 */
...
/* Função que escreve uma operação no buffer de memória partilhada entre os
 * intermediários e as empresas. A operação deve ser escrita numa posição livre
 * do buffer, tendo em conta o tipo de buffer e as regras de escrita em buffers
 * desse tipo. Se não houver nenhuma posição livre, não escreve nada.
 */
 ...
 * e os clientes, se houver alguma disponível para ler que seja direcionada ao
 * cliente especificado. A leitura deve ser feita tendo em conta o tipo de
 * buffer e as regras de leitura em buffers desse tipo. Se não houver nenhuma
 * operação disponível, afeta op->id com o valor -1.
 */
...
/* Função que lê uma operação do buffer de memória partilhada entre os clientes
 * e intermediários, se houver alguma disponível para ler (qualquer
 * intermediário pode ler qualquer operação). A leitura deve ser feita tendo em
 * conta o tipo de buffer e as regras de leitura em buffers desse tipo. Se não
 * houver nenhuma operação disponível, afeta op->id com o valor -1.
 */
...
/* Função que lê uma operação do buffer de memória partilhada entre os
 * intermediários e as empresas, se houver alguma disponível para ler dirijida à
 * empresa especificada. A leitura deve ser feita tendo em conta o tipo de
 * buffer e as regras de leitura em buffers desse tipo. Se não houver nenhuma
 * operação disponível, afeta op->id com o valor -1.
 */
